    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-th-list"></i> EDIT VENUE RESERVATION</h2>
            ...
            <div class="box-content">
                <!-- Start content here -->
					...
					<div class="alert alert-info">
						<a href="list_of_venue_reservation.php"><button class="btn btn-primary"><i class="glyphicon glyphicon-arrow-left"></i> Back</button></a>
					</div>

<?php
  $stmt = $sLink->query("select * from venue_schedule where id='$ID'");
  $row = $stmt->fetch(PDO::FETCH_ASSOC);                
                
  //$query=mysql_query("select * from venue_schedule where id='$ID'")or die(mysql_error());
...
?>					
					<form method="post" enctype="multipart/form-data" class="form-horizontal" style="margin-left:5px;">
                      ...
                      <div class="form-group">
                       ...
                       <div class="form-group">
						<label for="venue" class="col-sm-4 control-label">Venue to reserve</label>
						<div class="col-sm-6">
						  <select name="venue" class="form-control" required >
							<option><?php echo $row['room']; ?></option>
                              ...
                              <option>AVR C</option>
                              <option>AVR D</option>
                              <option>AVR A-B</option>
                              <option>AVR C-D</option>
                              <option>AVR A-B-C-D</option>
                              <option>VR 1</option>
                              <option>VR 2</option>
                              <option>VR 3</option>
                              <option>VR 4</option>
                              <option>VR 1-2</option>
                              <option>VR 3-4</option>
                              <option>VR 1-2-3-4</option>
                              <option>LR 1</option>
                              <option>LR 2</option>
                              <option>LR 3</option>
                              <option>LR 4</option>
                              <option>LR 1-2</option>
                              <option>LR 3-4</option>
                              <option>LR 1-2-3-4</option>
                              <option>Learning Commons Room</option>
                              <option>Preview Booth</option>
                              <option>Recording Booth</option>
...
{
                $sql = " UPDATE venue_schedule SET room='$venue',date_reservation='$date_reservation',time_reservation='$time_activity',purpose='$purpose',date_submitted='$date_updated',name='$name',id_number='$idnumber',contact='$contact',subject='$subject' WHERE id = '$id' ";
                $sLink->exec($sql);  
    
//mysql_query(" UPDATE venue_schedule SET room='$venue',date_reservation='$date_reservation',time_reservation='$time_activity',purpose='$purpose',date_submitted='$date_updated',name='$name',id_number='$idnumber',contact='$contact',subject='$subject' WHERE id = '$id' ")or die(mysql_error());
    
echo "<script>alert('Successfully updated venue reservation!'); window.location='list_of_venue_reservation.php'</script>";
    
						//mysql_query ("UPDATE locker SET venue_schedule (room,date_reservation,time_reservation,purpose,date_submitted,name,id_number,contact,subject) VALUES ('$venue','$date_reservation','$time_activity','$purpose','$date_submitted','$name','$idnumber','$contact','$subject')") or die(mysql_error());    
