    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-book"></i> HISTORY OF INTERNET UTILIZATION</h2>
             ...
             <div class="alert alert-info">
                ...
                <a href="print_loan_history_internet.php" target="_blank" style="background:none;">
                      <button class="btn btn-warning pull-right"> <i class="glyphicon glyphicon-print"></i> Print </button>
                </a> 
                 
                <a href="export_to_csv.php?action=20" target="_blank" style="background:none;">
						...
						<?php     
                            ...
                            if (isset($_POST['submit'])) {
								...
								if ($selected_radio == 'name') {
									//$result=mysql_query("select * from history where name like '%$keyword%' and time_out between '$start' and '$end'");
                                    $result = $sLink->query("select * from history where name like '%$keyword%' and time_out between '$date1' and '$date2'");  
								   } 
								elseif ($selected_radio == 'group') {
									//$result=mysql_query("select * from history where level like '%$keyword%' and (time_out between '$start' and '$end')");      
                                    $result = $sLink->query("select * from history where level like '%$keyword%' and time_out between '$date1' and '$date2'");  
								   } 
								elseif ($selected_radio == 'idnumber') {
									//$result=mysql_query("select * from history where client_id = '$keyword' and (time_out between '$start' and '$end')");          
                                    $result = $sLink->query("select * from history where client_id = '$keyword' and time_out between '$date1' and '$date2'");  
								   }	
								elseif ($selected_radio == 'type') {
									//$result=mysql_query("select * from history where level = '$keyword' and (time_out between '$start' and '$end')");           
                                    $result = $sLink->query("select * from history where client_id = '$keyword' and time_out between '$date1' and '$date2'");  
								   }
								else {
								   //$result= mysql_query("select * from history order by name,level,time_due") or die (mysql_error());
                                   $result = $sLink->query("select * from history order by name,level,time_out");   
								}
							}
							else {
							   //$result= mysql_query("select * from history order by name,level,time_in") or die (mysql_error());	
                               $result = $sLink->query("select * from history order by name,level,time_out");	
			...
			<tbody>
					...
					<tr>
				        ...
				        <td class="center">
                            ...
                            <i class="glyphicon glyphicon-eye-open icon-white"></i> View</a>
                            <?php include('modal_history_detail_internet.php'); ?>
                            
                            <a class="btn btn-danger" href="#delete<?php echo $id;?>" data-toggle="modal" data-target="#delete<?php echo $id;?>">
								<i class="glyphicon glyphicon-trash icon-white"> </i> Delete
							</a>
                            <?php include('modal_delete_history_internet.php'); ?>
						</td>
				    </tr>
					<?php } ?>
				</tbody>
