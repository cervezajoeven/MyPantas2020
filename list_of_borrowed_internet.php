    ...
    <div class="box col-md-12">
        <div class="box-inner">
            ...
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-refresh"></i> INTERNET USAGE MONITORING SYSTEM</h2>
						...
						<?php 
                            if (isset($_POST['submit'])) {
                                ...
                                elseif ($selected_radio == 'onhold') {
                                    ...
                                    $query = $sLink->query("select * from borrowing where client_id = '$idnum' and barcode = '$accession'");
                                    $row = $query->fetch();  
                                    $get_id=$row['id']; 
                                    $status='On hold'; 
                                    $charger = $_SESSION['charger'];
            						$date_reported = date("Y-m-d H:i:s"); 
                                    $sql = " UPDATE borrowing SET status='$status', charger='$charger', time_in='$date_reported' WHERE id = '$get_id' ";
                                    $sLink->exec($sql);  
                                ...
                                elseif ($selected_radio == 'return') {
                                    ...
                                    $accession=$_POST['accession'];
                                    $query = $sLink->query("select * from computer where barcode = '$accession'");
                                    $b_row = $query->fetch();
     
                                    $computer = $b_row['computer'];
                                    $barcode = $b_row['barcode'];
                                    $tmpid = $b_row['id'];   

                                    $borrowed_record = $sLink->query("select * from borrowing where client_id = '$idnum' and barcode = '$accession'");
                                    ...
                                    if ($dteDiff > 0){    
										...
										if ($dteDiff > 0){
											echo "<center><h1>This computer use is overdue of ", $dteDiff  , " pesos!</h1></center>";
										}
                                    }
                                    ...
                                    $sql = "INSERT INTO history (client_id, barcode, computer, name, level, time_out, time_due, time_in, charger) VALUES ('$idnum','$accession','$computer','$name','$type','$date_borrowed','$date_due','$date_returned','$charger')";
                                    $sLink->exec($sql); 
                                     

                                    $sql ="delete from borrowing where id = '$get_id' ";
                                    $sLink->exec($sql);   
                                    ...
                                    if ($selected_radio == 'borrow') {
                                        ...
                                        $query = $sLink->query("select * from borrowing where barcode = '$accession'");
                                        $results = $query->fetch();  
										...
										if($query) {								
                                            ...
                                            $result = $query->rowcount();
											if ($result > 0) {
												echo "<center><h1>This computer was already recorded as borrowed, kindly verify!</h1></center>";
												return;
											}
										}			
                                        ...
                                        $sLink->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                        $sql = "INSERT INTO borrowing (client_id, barcode, computer, time_out, time_due, charger, name, level) VALUES ('$idnum','$accession','$computer','$date_borrowed','$date_due','$charger','$name','$description')";
                                        $sLink->exec($sql); 
...
;
                                        ...
                                        if ($selected_radio == 'hourly'){
											...
											if($query) {								
                                                ...
                                                $result = $query->rowcount();
												if ($result > 0) {
													echo "<center><h1>This computer was already recorded as hourly borrowed, kindly verify!</h1></center>";
													return;
												}
											}
		...
		<table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
			...
			<tbody>
                    ...
                    if ($selected_radio == 'out_today') {
					   //$result= mysql_query("select * from borrowing where DATE_FORMAT(time_out, '%Y-%m-%d')=CURDATE()") or die (mysql_error());    
                       $result = $sLink->query("select * from borrowing where DATE_FORMAT(time_out, '%Y-%m-%d')=CURDATE()"); 
                        
                       } 
                    elseif ($selected_radio == 'due_today') {
					   //$result= mysql_query("select * from borrowing where DATE_FORMAT(time_due, '%Y-%m-%d')=CURDATE()") or die (mysql_error());    
                       $result = $sLink->query("select * from borrowing where DATE_FORMAT(time_due, '%Y-%m-%d')=CURDATE()");  
                       }               
                    else {
					   //$result= mysql_query("select * from borrowing order by name, client_id DESC ") or die (mysql_error());
                       $result = $sLink->query("select * from borrowing order by name, client_id DESC ");  
                    }
					...
					<tr>
				        ...
				        <td class="center">
                            ...
                            <?php include('modal_return_internet.php'); ?>
						</td>
				    </tr>
					<?php } ?>
				</tbody>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        ...
        <div class="modal-dialog">
            ...
            <center>
                        <a href="print_borrowed_internet.php" target="_blank" style="background:none;">
