						...
						<?php 
                            ...
                            if (isset($_POST['submit'])) {
								...
								if ($selected_radio == 'invbatch') {
                                    $action = 'On-shelf';
                                    if (empty($keyword)) {
                                        echo "<script>alert('You have to enter path or location of file for proccessing.'); window.location='list_of_inventory_thesis.php'</script>";
                                        return;
                                    }                                    
                                    ...
                                    if($count > 0) # file is not empty
                                    {
                                        foreach ($file as $row){
                                            $acc=trim($row);
                                            $stmt = $sLink->query("select * from td_inventory where accession = '$acc'");
                                            $ac_row = $stmt->fetch(PDO::FETCH_ASSOC);   
                                            $id=$ac_row['id'];
                                            $inventorydate=date("Y-m-d H:i:s");                                          

                                            $sql = " UPDATE td_inventory SET status='$action', idate='$inventorydate' WHERE id = '$id' ";
                                            $sLink->exec($sql); 
                                    ...
                                    echo "<script>alert($num + ' accession records processed successfully!'); window.location='list_of_inventory_thesis.php'</script>";
								} 
                                
								elseif ($selected_radio == 'invlist') {
                                    //$result=mysql_query("select * from acctd order by accession");  
                                    //while ($row= mysql_fetch_array ($result) ){
                                    $sql = "delete from td_inventory";
                                    // use exec() because no results are returned
                                    $sLink->exec($sql);
                                    
                                    $result = $sLink->query("select * from acctd order by accession");                         
                                    while ($row = $result->fetch() ){     
                                        ...
                                        $sql = "INSERT INTO td_inventory (accession,title,author,call_number,location) VALUES ('$accession', '$title','$author','$callnumber','$location')";
                                        $sLink->exec($sql);  
								...
								elseif ($selected_radio == 'invdate') {
                                    $result = $sLink->query("select * from td_inventory where idate between '$start' and '$end' order by accession");  
								...
								elseif ($selected_radio == 'invind') {
                                    ...
                                    $result = $sLink->query("select * from td_inventory where accession = '$keyword'");                         
                                    while ($row = $result->fetch() ) 
                                   ...
                                   $result = $sLink->query("select * from td_inventory order by accession");   
     ...
     $("#galileo").dataTable({
          ...
          error: function (xhr, error, thrown) {
            console.log(xhr);
                         ...
                         <a href="print_thesis_inventory_list.php" target="_blank" style="background:none;">
