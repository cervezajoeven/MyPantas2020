...
<div>
    <ul class="breadcrumb">
        ...
        <li>
            <a href="list_of_loan_history_locker.php">Locker</a>
        </li>
    </ul>
</div>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-book"></i> LIST OF LOCKER LOAN HISTORY</h2>
             ...
             <div class="alert alert-info">
                ...
                <a href="print_loan_history_locker.php" target="_blank" style="background:none;">
                      <button class="btn btn-warning pull-right"> <i class="glyphicon glyphicon-print"></i> Print </button>
                </a> 
                 
                        <a href="export_to_csv.php?action=22" target="_blank" style="background:none;">
						...
						<?php     
                            ...
                            if (isset($_POST['submit'])) {
								...
								if ($selected_radio == 'name') {
									//$result=mysql_query("select * from locker_history where name like '%$keyword%' and time_borrowed between '$date1' and '$date2'");
                                    $result = $sLink->query("select * from locker_history where name like '%$keyword%' and time_borrowed between '$date1' and '$date2'");  
								   } 
								elseif ($selected_radio == 'group') {
									//$result=mysql_query("select * from locker_history where level like '%$keyword%' and (time_borrowed between '$date1' and '$date2')");    
                                    $result = $sLink->query("select * from locker_history where level like '%$keyword%' and time_borrowed between '$date1' and '$date2'");  
								   } 
								elseif ($selected_radio == 'idnumber') {
									//$result=mysql_query("select * from locker_history where client_id = '$keyword' and (time_borrowed between '$date1' and '$date2')");    
                                    $result = $sLink->query("select * from locker_history where client_id = '$keyword' and time_borrowed between '$date1' and '$date2'");  
								   }	
								elseif ($selected_radio == 'type') {
									//$result=mysql_query("select * from locker_history where level = '$keyword' and (time_borrowed between '$date1' and '$date2')");    
                                    $result = $sLink->query("select * from locker_history where locker like '%$keyword%' and time_borrowed between '$date1' and '$date2'");  
								   }
								else {
								   //$result= mysql_query("select * from locker_history order by name,level,time_returned") or die (mysql_error());
                                   $result = $sLink->query("select * from locker_history where name like '%$keyword%' and time_borrowed between '$date1' and '$date2'");  
								}
							}
							else {
							   //$result= mysql_query("select * from locker_history order by name,level,time_returned") or die (mysql_error());		
                              $result = $sLink->query("select * from locker_history order by name, level, time_returned");   
							}
						?>							
					</form>				
		...
		<table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
			<thead>
				<tr>
					...
					<th width="10">Time borrowed</th>
					<th width="10">Time returned</th>   
			...
			<tbody>
					...
					<tr>
						...
						<td><?php echo $row['time_returned']; ?></td>                       
				        <td class="center">
                            ...
                            <?php include('modal_history_detail_locker.php'); ?>
                            ...
                            <?php include('modal_delete_history_locker.php'); ?>
						</td>
				    </tr>
					<?php } ?>
				</tbody>
        ...
        <script type="text/javascript">
         $("#galileo").dataTable({
               'bProcessing': true,
                ...
                "ajax":{
                url :"data_loan_history_locker.php",
