    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-book"></i> LIST OF SERIALS ISSUES RECEIVED</h2>
						...
						<?php 
                            ...
                            if (isset($_POST['submit'])) {
								...
								if ($selected_radio == 'name') {
                                    $result = $sLink->query("select * from serials where itemcode = '$keyword' and date_received between '$start' and '$end'");
									//$result=mysql_query("select * from serials where itemcode = '$keyword' and date_received between '$start' and '$end'");
								   } 
								...
								elseif ($selected_radio == 'idnumber') {
                                    $result = $sLink->query("select * from serials where idnum = '$keyword' and (date_received between '$start' and '$end')");
									//$result=mysql_query("select * from serials where idnum = '$keyword' and (date_received between '$start' and '$end')");           
								   }	
								elseif ($selected_radio == 'type') {
                                    $result = $sLink->query("select * from serials where status = '$keyword' and (date_received between '$start' and '$end')");
									//$result=mysql_query("select * from serials where status = '$keyword' and (date_received between '$start' and '$end')");           
								   }
								else {
                                   $result = $sLink->query("select * from serials where itemcode = '$keyword'"); 
								   //$result= mysql_query("select * from serials where itemcode = '$keyword'") or die (mysql_error());								   
								}
							}
							else {
                                $keyword=$_GET['itemcode'];
					...
					<tr>
				        ...
				        <td class="center">
                            ...
                            View
                                ...
                                <a class="btn btn-primary" href="edit_serials_issues.php<?php echo '?item_id='.$id; ?>">
                            ...
                            <?php include('modal_delete_serials_issue.php'); ?>
                            
                            
                                <a class="btn btn-success" href="edit_serials_issues_borrow.php<?php echo '?item_id='.$id; ?>">
                                    <span class="label-success label label-default"></span>
                                    <i class="glyphicon glyphicon-circle-arrow-up icon-white"></i> Borrow
                                </a>
                            
                            
                                <a class="btn btn-info" href="edit_serials_issues_return.php<?php echo '?item_id='.$id; ?>">
                                    <span class="label-warning label label-default"></span>
                                    <i class="glyphicon glyphicon-circle-arrow-down icon-white"></i> Return
                                </a>                           
