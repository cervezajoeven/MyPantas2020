    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-th-list"></i> EDIT AUDIO-VISUAL/SHELFLIST ITEM DETAILS</h2>
            ...
            <div class="box-content">
                <!-- Start content here -->
					...
					<div class="alert alert-info">
						<a href="list_of_accession_av.php"><button class="btn btn-primary"><i class="glyphicon glyphicon-arrow-left"></i> Back</button></a>
					</div>

<?php
  $stmt = $sLink->query("select * from accsm where id='$ID'");
  $row = $stmt->fetch(PDO::FETCH_ASSOC);                   
  //$query=mysql_query("select * from accsm where id='$ID'")or die(mysql_error());
...
{
$sql = " UPDATE accsm SET accession='$accession', title='$title',author='$author',publisher='$publisher',call_number='$callnumber',ye_ar='$taon',location='$location',status='$status',type='$mtype' WHERE id = '$id' ";
$sLink->exec($sql);
//mysql_query(" UPDATE accsm SET accession='$accession', title='$title',author='$author',publisher='$publisher',call_number='$callnumber',ye_ar='$taon',location='$location',status='$status',type='$mtype' WHERE id = '$id' ")or die(mysql_error());
    
echo "<script>alert('Successfully updated AV accession item information!'); window.location='list_of_accession_av.php'</script>";
