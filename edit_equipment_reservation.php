    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-th-list"></i> EDIT EQUIPMENT RESERVATION</h2>
            ...
            <div class="box-content">
                <!-- Start content here -->
					...
					<div class="alert alert-info">
						<a href="list_of_equipment_reservation.php"><button class="btn btn-primary"><i class="glyphicon glyphicon-arrow-left"></i> Back</button></a>
					</div>

<?php
  //$query=mysql_query("select * from equipmentnew_schedule where id='$ID'")or die(mysql_error());
  //$row=mysql_fetch_array($query);
  $stmt = $sLink->query("select * from equipmentnew_schedule where id='$ID'");
  $row = $stmt->fetch(PDO::FETCH_ASSOC);  
  $date1 = strtr($row['date_reservation'], '/', '-');
...
?>					
					<form method="post" enctype="multipart/form-data" class="form-horizontal" style="margin-left:5px;">
					  ...
					  <div class="form-group">
						...
						<div class="col-sm-6">
						  <input type="date" name="date_reservation" value="<?php echo $date_reservation; ?>" class="form-control" id="date_reservation" required />
						</div>
					  </div>
                      ...
                      <div class="form-group">
						...
						<div class="col-sm-6">
						  <select name="time_activity" class="form-control" required >
							<option><?php echo $row['time_reservation']; ?></option>
					  ...
					  <div class="form-group">
						<label for="name" class="col-sm-4 control-label">Faculty/Teacher</label>
						<div class="col-sm-6">
						  <input type="text" name="name" value="<?php echo $row['name']; ?>" class="form-control" id="name" placeholder="Name...." required />
						</div>
					  </div> 
 					  ...
 					  <div class="form-group">
						...
						<div class="col-sm-6">
						  <input type="text" name="idnumber" value="<?php echo $row['id_number']; ?>" class="form-control" id="idnumber" placeholder="ID number..." required />
						</div>
					  </div>                       
 					  ...
 					  <div class="form-group">
						...
						<div class="col-sm-6">
						  <input type="text" name="contact" value="<?php echo $row['contact']; ?>" class="form-control" id="contact" placeholder="Contact number..." required />
						</div>
					  </div>
						...
						<div class="col-sm-6">
						  <select name="subject" class="form-control" required >
                            ...
                            <option>BAM-101	 MARKETING RESEARCH</option>
                            <option>BAM-102	 PRICING STRATEGY</option>
                            <option>BAM-103	SALES MANAGEMENT</option>
                            <option>BAM-104	ADVERTISING AND PROMOTIONS MANAGEMENT</option>
                            <option>BAM-105	INTERNATIONAL MARKETING</option>
                            <option>BAM-106	ADVANCED MARKETING ISSUES</option>
                            <option>BAM-107 BAND MANAGEMENT</option>
                            <option>BAM-109	DIRECT MARKETING</option>
                            <option>BL-101	BUSINESS LAW</option>
                            <option>BL-102	LAW ON OBLIGATIONS AND CONTRACTS</option>
                            <option>BL-103	LAW ON BUSINESS ORGANIZATIONS</option>
                            <option>BL-104	LAW ON NEGOTIABLE INSTRUMENTS</option>
                            <option>BL-105	SALES AGENCY LABOR AND OTHER COMMERCIAL LAWS</option>
                            <option>BSA-33	MARKETING</option>
                            <option>BSA-44	BUSINESS POLICY</option>
                            <option>BSA-101	FUNDAMENTALS OF ACCOUNTING I</option>
                            <option>BSA-102	FUNDAMENTALS OF ACCOUNTING II</option>
                            <option>BSA-103	FINANCIAL ACCOUNTING AND REPORTING I</option>
                            <option>BSA-104	FINANCIAL ACCOUNTING AND REPORTING II</option>
                            <option>BSA-105	FINANCIAL ACCOUNTING AND REPORTING III</option>
                            <option>BSA-106	FINANCIAL ACCOUNTING AND REPORTING IV</option>
                            <option>BSA-107	COST ACCOUNTING AND COST MANAGEMENT I</option>
                            <option>BSA-108	COST ACCOUNTING AND COST MANAGEMENT II</option>
                            <option>BSA-109	ADVANCED FINANCIAL ACCOUNTING AND REPORTING I</option>
                            <option>BSA-110	ADVANCED FINANCIAL ACCOUNTING AND REPORTING II</option>
                            <option>BSA-113	ACCTG FOR GOVT NOT-FOR-PROF. ENTTITIES and SPEC. IND.</option>
                            <option>BSA-114	ASSURANCE PRINCIPLE PROFFESIONAL ETHICS AND GOOD GOVERNANCE</option>
                            <option>BSA-115	APPLIED AUDITING</option>
                            <option>BSA-117	MANAGEMENT ACCOUNTING I</option>
...
$sql = " UPDATE equipmentnew_schedule SET equipment='$equipment',date_reservation='$date_reservation',time_reservation='$time_activity',purpose='$purpose',date_submitted='$date_updated',name='$name',id_number='$idnumber',contact='$contact',subject='$subject' WHERE id = '$id' ";
$sLink->exec($sql);  
//mysql_query(" UPDATE equipmentnew_schedule SET equipment='$equipment',date_reservation='$date_reservation',time_reservation='$time_activity',purpose='$purpose',date_submitted='$date_updated',name='$name',id_number='$idnumber',contact='$contact',subject='$subject' WHERE id = '$id' ")or die(mysql_error());
    
echo "<script>alert('Successfully updated equipment reservation!'); window.location='list_of_equipment_reservation.php'</script>";
